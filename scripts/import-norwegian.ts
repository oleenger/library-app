// One-off import of owned Norwegian-language ebooks as ELECTRONIC editions.
//
// Same mechanics as scripts/import-kindle.ts (see that file for the full
// rationale): reads a self-contained, already-categorised CSV
// (data/norwegian_ebooks_fiction_categorized.csv, format=ebook,
// edition_language=Norwegian) and upserts straight to Supabase. It does NOT
// touch the deprecated master CSV.
//
...
//     re-running adds nothing and an ebook never merges into a print edition.
// After the upsert it replays the Goodreads shelf so any newly-owned title you
// have read is marked read immediately.
//
// Run with (Node 20.6+ loads the env file itself):
//   node --env-file=.env.local --import tsx scripts/import-norwegian.ts
...
const SOURCE = path.join(ROOT, "data", "norwegian_ebooks_fiction_categorized.csv");

async function main(): Promise<void> {
  if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) {
    console.error(
      "Missing Supabase env. Run: node --env-file=.env.local --import tsx scripts/import-norwegian.ts",
    );
    process.exit(1);
  }
  ...
  console.log("Norwegian ebook import (electronic editions):");
  ...
  // Retro-match the Goodreads shelf against the now-larger catalogue.
